#/bin/bash
cd /home/sunlinlin/report_zuoye/report/

d=$1
d1=`date -d "$d -1 day" +%Y%m%d` #20170403
d7=`date -d "$d -7 day" +%Y%m%d` #20170328
d8=`date -d "$d -8 day" +%Y%m%d` #20170327
d14=`date -d "$d -14 day" +%Y%m%d` #20170321
d21=`date -d "$d -21 day" +%Y%m%d` #20170314
echo $d1
hive -e "set hive.exec.parallel=true;
drop table test.deco_table1;
create table  test.deco_table1
as
...
(
	...
	from 
	(  
			select date_sort dt from dm.d_date where date_sort >= ${d14} and date_sort <= ${d1}
		) a
	left outer join 
	(
